import time

from celery import shared_task
from objects.models.stores import EccommerceStore


@shared_task(name='initial_discovery_task')
def initial_discovery():
    from scraper.stores.leroy_merlin.leroy_merlin import LeroyMerlinScraper
    with LeroyMerlinScraper(requested_url="https://www.new.leroymerlin.pl/", store_url="https://www.leroymerlin.pl/") as scraper:
        element = scraper.visit_page(url=scraper.requested_url)
        generator = scraper.find_categories_data(category_level=0, html_element=element)
        for cat in generator:
            print(cat)
        scraper.quit_and_clean()


@shared_task(name='discovery_task')
def discovery_task():
    # discovery_group = group(
    #     [initial_discovery() for _ in range(12)]
    #     ).apply_async()
    for _ in range(12):
        initial_discovery.apply_async()
    # return discovery_group


@shared_task(
        bind=True,
        autoretry_for=(Exception,),
        retry_backoff=60,
        name='create_initial_discovery_tasks',
    )
def create_initial_discovery_tasks(self):
    active_stores = EccommerceStore.objects.filter(is_active=True)
    for store in active_stores:
        created = time.time()
        task = TaskObject.objects.create(store=store, status='CREATED', type='CRAWL', created=created)
    return task.id